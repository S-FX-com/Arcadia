...
async function getCachedReport(teamId: string, channelId: string, weekStart: string, env: Env): Promise<string | null> {
	return env.ARCADIA_CACHE.get(weeklyCacheKey(teamId, channelId, weekStart));
}

async function cacheReport(teamId: string, channelId: string, weekStart: string, content: string, env: Env): Promise<void> {
 ...
 */
async function compileWeeklyStats(teamId: string, channelId: string, env: Env): Promise<WeeklyTaskStats> {
	...
	const deadlinesMissed = openTasks.filter((t) => t.deadline !== null && t.deadline < now).length;
 ...
 */
export async function generateWeeklyReport(channel: ChannelRow, env: Env): Promise<string> {
	...
	const language = messages.length > 0 ? detectConversationLanguage(messages) : "en";

	const stats = await compileWeeklyStats(channel.team_id, channel.channel_id, env);

	const { system, user } = buildWeeklyReportPrompt(channel.channel_name, weekStart, stats, messages, language);
 ...
 */
function buildStaticWeeklyReport(channelName: string, weekStart: string, stats: WeeklyTaskStats): string {
	const lines = [
 ...
 */
export async function postWeeklyReport(channel: ChannelRow, env: Env): Promise<void> {
	if (!channel.service_url || !channel.conversation_id) {
		console.warn(`[Arcadia/Weekly] No service URL for ${channel.channel_id} — skipping post`);
		return;
	}
	...
	if (!tokenRes.ok) {
		...
		console.error("[Arcadia/Weekly] Bot token fetch failed:", tokenRes.status, err);
		return;
	}
	...
	if (!res.ok) {
		...
		console.error("[Arcadia/Weekly] Post failed:", res.status, body);
		try {
			const parsed = JSON.parse(body);
			if (res.status === 403 && parsed?.error?.message?.includes("BotNotInConversationRoster")) {
				console.warn(`[Arcadia/Weekly] Bot not in conversation roster for ${channel.channel_id}, unregistering channel.`);

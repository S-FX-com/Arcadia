 ...
 */
export async function isNudgeRateLimited(
  taskId: string,
  env: Env
): Promise<boolean> {
...
export function buildNudgeCandidates(
  ...
  return Array.from(byId.values()).sort(
    (a, b) => URGENCY_RANK[a.reason] - URGENCY_RANK[b.reason]
  );
}
...
export function buildStaticNudge(candidate: NudgeCandidate): string {
  ...
  switch (reason) {
    ...
    case "deadline-24h":
      lines.push(
        `Due in less than 24 hours. ${owner} — please confirm status or flag if blocked.`
      );
      break;
    case "deadline-48h":
 ...
 */
async function postNudgeToChannel(
  channel: ChannelRow,
  text: string,
  env: Env
): Promise<void> {
  if (!channel.service_url || !channel.conversation_id) {
    console.warn(
      `[Arcadia/Nudge] No service URL for ${channel.channel_id} — skipping proactive post`
    );
    return;
  }

  // Get Bot Framework bearer token
  ...
  if (!tokenRes.ok) {
    console.error("[Arcadia/Nudge] Bot token fetch failed:", tokenRes.status);
    return;
  }
  ...
  if (!res.ok) {
    console.error("[Arcadia/Nudge] Post failed:", res.status, await res.text());
  }
}
...
export async function runNudgeEngine(env: Env): Promise<void> {
  ...
  console.log(
    `[Arcadia/Nudge] Starting nudge engine (cooldown: ${cooldownHours}h, max: ${maxPerRun})`
  );

  // Fetch tasks needing nudges across all channels
  const [staleTasks, dueSoonTasks] = await Promise.all([
    getTasksNeedingNudge(cooldownSeconds, env),
    getTasksDueWithin(48, env),
  ]);
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      console.log(
        `[Arcadia/Nudge] Nudged task ${task.id} (${candidate.reason}): ${task.description.slice(0, 50)}`
      );
    } catch (err) {
